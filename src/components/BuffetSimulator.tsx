    ...
    <div id="buffet-simulator" className="w-full glass-panel rounded-2xl p-6 md:p-8 border border-emerald-500/10 relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 border-b border-zinc-900 pb-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <span className="text-[11px] md:text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded font-bold px-1.5 py-0.5 uppercase tracking-wide inline-block mb-1">MÓDULO EXCLUSIVO 🚀</span>
            ...
            <p className="text-sm md:text-xs text-slate-400 max-w-2xl mt-1 leading-relaxed">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-6 space-y-5">
          ...
          <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-4 space-y-4">
            <span className="text-sm md:text-xs font-bold text-white block uppercase tracking-wider font-mono">1. Divisão da Rampa (Mix de Consumo)</span>
            
            {/* Leves */}
            <div>
              <div className="flex justify-between items-center text-sm md:text-xs mb-1">
            ...
            {/* Total Indicator */}
            <div className="flex justify-between items-center text-xs md:text-[10px] text-slate-500 font-mono pt-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-4">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm md:text-xs font-bold text-white uppercase font-mono">Desperdício Cuba</span>
                <span className="text-sm md:text-xs text-emerald-400 font-mono font-bold">{leftoverLoss.toFixed(1)}%</span>
              </div>
              <input
                type="range"
                min="0"
              ...
              />
              <span className="text-xs md:text-[9px] text-slate-500 mt-1 block">Sobras de comida jogadas fora no fechamento de rampa.</span>
            </div>

            <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-4">
              <div className="flex justify-between items-center mb-1">
                <span className="text-sm md:text-xs font-bold text-white uppercase font-mono">Quebra Térmica</span>
                <span className="text-sm md:text-xs text-emerald-400 font-mono font-bold">{evaporation}%</span>
              </div>
              <input
                type="range"
                min="0"
                ...
                className="w-full accent-emerald-500 bg-slate-800 h-1 rounded"
              />
              <span className="text-xs md:text-[9px] text-slate-500 mt-1 block">Evaporação de água da comida sob as lâmpadas quentes.</span>
            </div>
        ...
        <div className="lg:col-span-6 flex flex-col justify-between space-y-4">
          ...
          <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-5 space-y-4 flex-1 flex flex-col justify-between">
            <div>
              <span className="text-[11px] md:text-[10px] text-slate-500 uppercase tracking-wider block font-mono">Preço Ideal Sugerido para o Caixa (Margem de {targetMargin}% + {simplesNational}% imposto)</span>
              ...
              <p className="text-sm md:text-xs text-slate-400 leading-relaxed mt-1.5">
            ...
            <div className="pt-4 border-t border-slate-800 grid grid-cols-2 gap-4">
              <div>
                <span className="text-[11px] md:text-[10px] text-slate-500 uppercase tracking-wider block font-mono">Custo Kg Ponderado (Sem perdas)</span>
                <span className="text-lg font-mono font-bold text-slate-300">R$ {results.weightedCostKg.toFixed(2)}</span>
                <span className="text-xs md:text-[9px] text-slate-500 block">Mix proporcional de consumo</span>
              </div>

              <div>
                <span className="text-[11px] md:text-[10px] text-slate-500 uppercase tracking-wider block font-mono">Custo Kg Real (Com perdas de evaporação e cubas)</span>
                <span className="text-lg font-mono font-bold text-emerald-400">R$ {results.operationalCostKg.toFixed(2)}</span>
                <span className="text-xs md:text-[9px] text-slate-500 block">Evaporação + desperdício de cubas</span>
              </div>
            </div>
            ...
            <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-lg p-3 flex items-start gap-2.5">
              ...
              <div>
                <h5 className="text-xs md:text-[11px] font-bold text-white">Preço Recomendado para Buffet Livre (Coma à Vontade)</h5>
                <p className="text-xs md:text-[11px] font-mono text-emerald-400 font-bold">R$ {results.suggestedBuffetLivre.toFixed(2)} por pessoa</p>
                <p className="text-xs md:text-[10px] text-slate-400 leading-relaxed mt-1">
